<template>
  <div>
    ...
    <!-- 作者沒有收藏文章 -->
    <div
      v-if="collectCommon.length === 0 && isOpen && !isLoading"
...
export default {
  ...
  computed: {
    sortHandler() {
      let sortList = []
      if (this.defaultSort) {
        sortList = [...this.collectCommon].sort((a, b) => {
  ...
  watch: {
    'paginationVm.currentPage': {
      handler(newPage) {
        this.getAuthorCollectCommonHandler(newPage)
      },
    },
  },
  mounted() {
    this.getAuthorCollectCommonHandler(this.paginationVm.currentPage)
  },
  methods: {
    getAuthorCollectCommonHandler(currentPage) {
